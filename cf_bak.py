...
def model_and_predict(rate_matrix,user_ids_list,item_ids_list,top_n,fout_str):
    ...
    np.save('user_dis_buy_50',user_distribution)
    np.save('item_dis_buy_50',item_distribution)
...
def direct_predict_item_base(rate_matrix,user_ids_list,item_ids_list,top_n,fout_str):
    ...
    for u_ix,user in enumerate(user_ids_list[:10]):
        if u_ix %100 ==0:
            print u_ix
        candidate_item_list = [] 
        print '---------------------'
        ...
        candiadte_items = heapq.nlargest(2,candidate_item_list,key = lambda x:x[1])
